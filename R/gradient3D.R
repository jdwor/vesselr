...
gradient3D=function(image,which="all"){
  if(is.nifti(image)){
    if(which=="all"){
      dx=image
      dx@.Data[1,,]=0
    ...
    if(which=="all"){
      ...
      dx[1,,]=0
      dx[dim(image)[1],,]=0
      dx[(2:(dim(image)[1]-1)),,]=(image[(3:dim(image)[1]),,]-
                                           image[(1:(dim(image)[1]-2)),,])/2

      dy=image
      dy[,1,]=0
      dy[,dim(image)[2],]=0
      dy[,(2:(dim(image)[2]-1)),]=(image[,(3:dim(image)[2]),]-
                                           image[,(1:(dim(image)[2]-2)),])/2

      dz=image
      dz[,,1]=0
      dz[,,dim(image)[3]]=0
      dz[,,(2:(dim(image)[3]-1))]=(image[,,(3:dim(image)[3])]-
                                           image[,,(1:(dim(image)[3]-2))])/2
